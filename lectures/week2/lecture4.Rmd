...
head(fuel)
pairs(fuel %>% dplyr::select(-pop,-state)) 
```
...
This assymmetry causes the p-value of an F-Statistic to get very small as $k$ (the number of predictors) grows large. Thus for even moderate values of $k$, the $F-statistic$ might be considered to have an outsized false positive rate. There are ways to account for this (such as the [Bonferroni Correction](https://en.wikipedia.org/wiki/Bonferroni_correction)), but my opinion this property makes F-tests more trouble than they're worth.
...
You will notice in the regression table, that R has assigned p-values to all the coefficients individually, not just the F-statistic for the whole model.
