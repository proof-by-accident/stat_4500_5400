...
mod = stan_glm(survived~pclass+sex+age+sibsp+parch+fare, data=dat, family=binomial, prior=normal(location=0,scale=2.5), refresh=0) # rstanarm auto-scales independent variables and converts 
summary(mod)
plot(mod)
```

We can also look at what happens to our estimates under a more diffuse prior:

```{r}
mod.diffuse = stan_glm(survived~pclass+sex+age+sibsp+parch+fare, data=dat, family=binomial, prior=normal(location=0,scale=100), refresh=0 ) 
mod.diffuse.lowN = stan_glm(survived~pclass+sex+age+sibsp+parch+fare, data=dat%>%head(50), family=binomial, prior=normal(location=0,scale=100), refresh=0 ) 
...
```{r}
mod.hiearchical = stan_glmer(survived~(sex+age+fare+parch)|pclass, data=dat, family=binomial 
                             , refresh=0)

loo.hierarchical = loo(mod.hiearchical)
loo_compare(loo.noparch, loo.hiearchical) # this comparison includes smart estimates of the error in LOO-CV differences
```
